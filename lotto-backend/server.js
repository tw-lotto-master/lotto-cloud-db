...
app.post('/api/lottery/generate-vip-turbo', async (req, res) => {
 ...
 // 【前置全域變數外掛化】：擊穿大口袋 Scope 阻斷，還原前台流暢推進 🔑
 let totalScanned = 0;
 let matchCount = 0;
           ...
           for (let i4 = i3 + 1; i4 <= 38; i4++) {
             for (let i5 = i4 + 1; i5 <= 39; i5++) {
               ...
               if (isCombValid && cfg.f13_on) {
                 ...
                 if (acVal < (parseInt(cfg.f13_min, 10) || 1)) isCombValid = false;
               }
               ...
               if (isCombValid && cfg.f15_on && cfg.f15_kill) {
                 ...
                 if (typeof globalHistoryBigInts !== 'undefined' && globalHistoryBigInts.length > 0) {
                   for (let h = 0; h < globalHistoryBigInts.length; h++) {
                     ...
                     if (matchOverlap >= overlapLimit) { isOverlapLimit = true; break; }
                   }
                 }
...
// =========================================================================
    ...
    // 精密自癒防禦門：完美對齊並咬住大樂透專區 else 大口袋的 try 開頭！
...
} // 【第一道金鎖】：完美閉合第 13 頁最外層大樂透的 else { 大口袋大門！ 🔒

} catch (globalErr) {
  // 覆蓋全路由的最高生命線安全氣囊，完美對齊並咬住接口最頂層的 try 開頭！
  console.error(" 全局路由異常：", globalErr.message);
  try { res.end(); } catch(e){}
}
}); // 【第 309 行總防護罩】：精確對齊並重啟 app.post('/api/lottery/generate-vip-turbo') 大門！ 🔒
// =========================================================================
